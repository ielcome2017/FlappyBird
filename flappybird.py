...
class Game(QWidget):
    def __init__(self, parent=None, flags=Qt.WindowFlags()):
        ...
        self.resize(288, 427)
        # self.img_pipe = QPixmap()
        # self.img_pipe.load("assets/sprites/pipe-green.png")
...
class Main(QWidget):
    ...
    def __init__(self, parent=None, flags=Qt.WindowFlags()):
        ...
        self.game = Game(self)
        self.game.resize(self.size())
        self.scoreboard = Main.Scoreboard()
        self.game.setContentsMargins(0, 0, 0, 0)
        self.scoreboard.setContentsMargins(0, 0, 0, 0)

        layout = QtWidgets.QVBoxLayout()
        layout.setSpacing(0)
        layout.addWidget(self.game, stretch=5)
        layout.addWidget(self.scoreboard, stretch=1)
        layout.setContentsMargins(0, 0, 0, 0)
        self.setLayout(layout)

        for i in range(1000):
            act = np.random.randint(0, 2, size=[2])
            self.game.frame_step(act)
            
        
        # self.sign_step.connect(self.game.step)
        # self.game.run()
